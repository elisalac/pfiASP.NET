...
namespace ChatManager.Controllers
{
    public class AccountsController : Controller
    {
        ...
        {
            List<Chat> chats = DB.Chats.ToList().Where(u => u.idAmi == userid || u.idUser == userid).ToList();
            List<Friendship> friendships = DB.Friendships.ToList().Where(f => f.recievingId == userid || f.requestingId == userid).ToList();
